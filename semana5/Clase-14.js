...
window.addEventListener('load', function () {
    ...
    // utilizamos la funcion para el boton de cerrar sesión
    botonCerrarSesion();
})
...
function botonCerrarSesion() {
    //capturamos el div donde colocar el boton
    let divbtn = document.querySelector('div.user');
    divbtn.innerHTML += `
    <button type="button" class="btn-cerrar-sesion">Cerrar sesión</button> 
    `
    divbtn.addEventListener('click', function() {
        let eleccion = confirm('¿Seguro desea cerrar sesión?');
        if (eleccion) {
            localStorage.clear();
